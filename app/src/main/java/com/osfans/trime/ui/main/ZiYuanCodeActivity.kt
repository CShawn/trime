...
class ZiYuanCodeActivity : Activity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val code = genCode(text, dictFile)
        if (code.isEmpty()) {
        ...
        appendText(text, customFile)
        writeDict(text, code, dictFile)
        Handler(Looper.getMainLooper()).postDelayed({ finish() }, 800)
    }

    private fun appendText(text: String, customFile: File) {
    ...
    private fun genCode(text: String, dictFile: File): List<String> {
    ...
    private fun writeDict(text: String, codes: List<String>, dictFile: File) {
        // 遍历codes，将"{text}\t\{code}\n"追加写入dictFile
        if (!dictFile.exists()) {
